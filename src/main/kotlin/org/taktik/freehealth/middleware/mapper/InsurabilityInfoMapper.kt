...
fun GetInsurabilityResponse.toInsurabilityInfoDto(): InsurabilityInfoDto = InsurabilityInfoDto(
        inss = response.careReceiverDetail?.inss,
        ...
        specialSocialCategory = response?.insurabilityResponseDetail?.payment?.isSpecialSocialCategory ?: false
)
...
private fun PeriodType.toPeriodDto(): PeriodDto? = PeriodDto(
        periodStart = periodStart?.let { FuzzyValues.getFuzzyDate(it) },
...
fun MedicalHouseType.toMedicalHouseInfoDto(): MedicalHouseInfoDto = MedicalHouseInfoDto(
        periodStart = periodStart?.let { FuzzyValues.getFuzzyDate(it) },
        periodEnd = periodEnd?.let { FuzzyValues.getFuzzyDate(it) },
        isNurse = isNurse,
        isMedical = isMedical,
