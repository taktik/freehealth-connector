...
class AddressbookServiceImpl(val stsService: STSService) : AddressbookService {
    private val freehealthTokenAddressbookService: org.taktik.connector.business.addressbook.service.AddressbookTokenService = org.taktik.connector.business.addressbook.service.impl.AddressbookTokenServiceImpl(SAMLSessionValidator(), EhealthReplyValidatorImpl())
    override fun searchHcp(keystoreId: UUID, tokenId: UUID, passPhrase: String, queryLastName: String, queryFirstName: String?, type: String): List<HealthcareParty> {
        ...
        val searchProfessionals = freehealthTokenAddressbookService.searchProfessionals(samlToken, SearchProfessionalsRequest().apply {
            firstName = queryFirstName; lastName = queryLastName; issueInstant = DateTime.now(); profession = type
            offset = 0; maxElements = 100
        })
        return searchProfessionals.healthCareProfessionals.filter { queryFirstName == null || it.firstName != null && it.firstName.startsWith(queryFirstName, true) }.map {
            HealthcareParty(
                    firstName = it.firstName,
                    lastName = it.lastName,
                    ssin = it.ssin,
                    nihii = (it.professions.find { it.professionCodes.any { it.value == "PHYSICIAN" } } ?: it.professions.firstOrNull())?.nihii,
                    gender = Gender.fromCode(it.gender) ?: Gender.undefined)
        }
    }

    override fun searchOrg(keystoreId: UUID, tokenId: UUID, passPhrase: String, name: String, type: String): List<HealthcareParty> {
        val samlToken = stsService.getSAMLToken(tokenId, keystoreId, passPhrase) ?: throw IllegalArgumentException("Cannot obtain token for Ehealth Box operations")
        val searchProfessionals = freehealthTokenAddressbookService.searchOrganizations(samlToken, SearchOrganizationsRequest().apply {
    ...
    override fun getHcp(keystoreId: UUID, tokenId: UUID, passPhrase: String, nihii: String?, ssin: String?, language: String): HealthcareParty {
        val samlToken = stsService.getSAMLToken(tokenId, keystoreId, passPhrase) ?: throw IllegalArgumentException("Cannot obtain token for Ehealth Box operations")
        val professionalContactInfo = freehealthTokenAddressbookService.getProfessionalContactInfo(samlToken, GetProfessionalContactInfoRequest().apply {
    ...
    override fun getOrg(keystoreId: UUID, tokenId: UUID, passPhrase: String, ehp: String?, cbe: String?, nihii: String?, language: String): HealthcareParty {
        val samlToken = stsService.getSAMLToken(tokenId, keystoreId, passPhrase) ?: throw IllegalArgumentException("Cannot obtain token for Ehealth Box operations")
        val professionalContactInfo = freehealthTokenAddressbookService.getOrganizationContactInfo(samlToken, GetOrganizationContactInfoRequest().apply {
    ...
    private fun makeHealthcareParty(it: IndividualContactInformationType, language: String): HealthcareParty {
        ...
        return HealthcareParty(firstName = it.firstName,
                lastName = it.lastName,
                ssin = it.ssin,
                gender = Gender.fromCode(it.gender) ?: Gender.undefined,
            ...
            addresses.addAll(professionalInformation?.addresses?.map {
                Address(
                    ...
                    telecoms.addAll(professionalInformation.healthCareAdditionalInformations?.filter { it.type == "Mail" }?.map { Telecom(telecomType = TelecomType.email, telecomNumber = it.value) } ?: listOf())
                }
            } ?: listOf())
        }
    }
    ...
    private fun makeHealthcareParty(org: OrganizationContactInformationType, language: String): HealthcareParty {
        return HealthcareParty(
                name = org.names.find { language == it.lang }?.value ?: org.names.joinToString { it.value },
            ...
            addresses.addAll(org.addresses?.map {
                Address(
                    ...
                    telecoms.addAll(org.healthCareAdditionalInformations?.filter { it.type == "Mail" }?.map { Telecom(telecomType = TelecomType.email, telecomNumber = it.value) } ?: listOf())
                }
            } ?: listOf())
        }
    }
