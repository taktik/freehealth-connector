...
fun HttpURLConnection.postForm(form: List<Pair<String, String>>): InputStream? {
    ...
    outputStream.bufferedWriter().use {
        it.write(form.map { (k, v) -> "${k}=${java.net.URLEncoder.encode(v, Charsets.UTF_8.name())}" }
                     .joinToString("&")
                )
    }
...
class RswFhirServiceImpl(val stsService: STSService) : RswFhirService {
    ...
    private fun getJwtTokenContext(rswPublicKey: PublicKey, jwtToken: String): JwtContext? {
        val firstPassConsumer = JwtConsumerBuilder()
            ...
            .setAllowedClockSkewInSeconds(120).build()
        return firstPassConsumer.process(jwtToken) //Will throw an exception if invalid
    }
    ...
    override fun search(nihii: String,
        ...
        val accessToken = obtainAccessToken(clientId, clientSecret, nihii)
        ...
        } ?: throw IllegalStateException("Cannot decode")
    }
    ...
    private fun obtainAccessToken(clientId: String, clientSecret: String, nihii: String) {
        (URL("https://jacc.reseausantewallon.be/is4acc/careset/v1/token").openConnection() as? HttpURLConnection)?.postForm( //Request token
            ...
            objectMapper.readValue(readText, Jwt::class.java)?.accessToken?.also { at ->
                getJwtTokenContext(fetchRSWPublicKey(), at)
            }
        }
    }
