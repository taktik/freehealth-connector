...
class TherLinkServiceImpl(private val stsService: STSService) : TherLinkService {
	...
	override fun getAllTherapeuticLinks(keystoreId: UUID, tokenId: UUID, passPhrase: String, hcpNihii: String, hcpSsin: String, hcpFirstName: String, hcpLastName: String, patientSsin: String, patientFirstName: String, patientLastName: String, eidCardNumber: String?, isiCardNumber: String?, startDate: Date?, endDate: Date?, type: String?, sign: Boolean?): List<TherapeuticLinkMessage>? =
			getAllTherapeuticLinksWithQueryLink(keystoreId, tokenId, passPhrase,
					TherapeuticLink.Builder().withHcParty(makeHcParty(hcpNihii, hcpSsin, hcpFirstName, hcpLastName))
							.withPatient(makePatient(patientSsin, eidCardNumber, isiCardNumber, patientFirstName, patientLastName))
							.withStartDateTime(startDate?.let { DateTime(it.time) }).withEndDateTime(endDate?.let { DateTime(it.time) })
							.withStatus(TherapeuticLinkStatus.ACTIVE)
							.withType(type ?: "gpconsultation")
							.build(),
					sign)

	override fun getAllTherapeuticLinksWithQueryLink(keystoreId: UUID, tokenId: UUID, passPhrase: String, queryLink: TherapeuticLink, sign: Boolean?): List<TherapeuticLinkMessage>? =
			getAllTherapeuticLinksWithQueryLink(keystoreId, tokenId, passPhrase, queryLink, makeProof(sign ?: false, queryLink.patient, queryLink.hcParty))

	private fun getAllTherapeuticLinksWithQueryLink(keystoreId: UUID, tokenId: UUID, passPhrase: String, queryLink: TherapeuticLink, proof: Proof?): List<TherapeuticLinkMessage>? {
		...
		val response = org.taktik.connector.technical.ws.ServiceFactory.getGenericWsSender().send(getTherLinkPort(samlToken).apply {
			setSoapAction("urn:be:fgov:ehealth:therlink:protocol:v1:GetTherapeuticLink")
			setPayload(requestObjectMapper.mapGetTherapeuticLinkRequest(GetTherapeuticLinkRequest(DateTime.now(), getNihii(queryLink.hcParty)!!, Author().apply {
				hcParties.add(queryLink.hcParty)
			}, queryLink, 100, proof)))
		}).asObject(GetTherapeuticLinkResponse::class.java)

		return if (response.acknowledge.isIscomplete) {
			responseObjectMapper.mapJaxbToGetTherapeuticLinkResponse(response).listOfTherapeuticLinks.map { TherapeuticLinkMessage(it) }
		} else {
			listOf(TherapeuticLinkMessage().apply {
				isComplete = response.acknowledge.isIscomplete
				errors = responseObjectMapper.mapAcknowledge(response.acknowledge).listOfErrors.filter { it.errorCode != "NIP.META.TlServiceBean" }.map { org.taktik.connector.business.domain.Error(it.errorCode, null, it.errorDescription, mapOf()) }
			})
		}
	}

	override fun doesLinkExist(keystoreId: UUID, tokenId: UUID, passPhrase: String, therLink: TherapeuticLink): TherapeuticLink? =
			getAllTherapeuticLinksWithQueryLink(keystoreId, tokenId, passPhrase, therLink, false)?.firstOrNull()?.let { if (it.isComplete) it.therapeuticLink else null }
	...
	override fun registerTherapeuticLink(keystoreId: UUID, tokenId: UUID, passPhrase: String, hcpNihii: String, hcpSsin: String, hcpFirstName: String, hcpLastName: String, patientSsin: String, patientFirstName: String, patientLastName: String, eidCardNumber: String?, isiCardNumber: String?, start: Date?, end: Date?, therLinkType: String?, comment: String?, sign: Boolean?): TherapeuticLinkMessage {
		val samlToken = stsService.getSAMLToken(tokenId, keystoreId, passPhrase)
				?: throw IllegalArgumentException("Cannot obtain token for Ehealth Box operations")
		val therLink = makeTherapeuticLink(therLinkType ?: "gpconsultation", makeHcParty(hcpNihii, hcpSsin, hcpFirstName, hcpLastName), makePatient(patientSsin, eidCardNumber, isiCardNumber, patientFirstName, patientLastName), start?.let { DateTime(it.time) }, end?.let { DateTime(it.time) }, comment)

		doesLinkExist(keystoreId, tokenId, passPhrase, therLink)?.let { revokeLink(keystoreId, tokenId, passPhrase, it, false) }

		val response = org.taktik.connector.technical.ws.ServiceFactory.getGenericWsSender().send(getTherLinkPort(samlToken).apply {
			setPayload(requestObjectMapper.mapPutTherapeuticLinkRequest(PutTherapeuticLinkRequest(DateTime(), hcpNihii, makeAuthor(hcpNihii, hcpSsin, hcpFirstName, hcpLastName), therLink,
	...
	override fun revokeLink(keystoreId: UUID, tokenId: UUID, passPhrase: String, hcpNihii: String, hcpSsin: String, hcpFirstName: String, hcpLastName: String, patientSsin: String, patientFirstName: String, patientLastName: String, eidCardNumber: String?, isiCardNumber: String?, start: Date?, end: Date?, therLinkType: String?, comment: String?, sign: Boolean?): TherapeuticLinkMessage? {
		return doesLinkExist(keystoreId, tokenId, passPhrase, makeTherapeuticLink(therLinkType?:"gpconsultation", makeHcParty(hcpNihii, hcpSsin, hcpFirstName, hcpLastName), makePatient(patientSsin, eidCardNumber, isiCardNumber, patientFirstName, patientLastName), start?.let { DateTime(it.time) }, end?.let { DateTime(it.time) }, comment))?.let { revokeLink(keystoreId, tokenId, passPhrase, it, sign) }
	}

	override fun revokeLink(keystoreId: UUID, tokenId: UUID, passPhrase: String, therLink: TherapeuticLink, sign: Boolean?): TherapeuticLinkMessage {
		val samlToken = stsService.getSAMLToken(tokenId, keystoreId, passPhrase)
				?: throw IllegalArgumentException("Cannot obtain token for Ehealth Box operations")

		val mapRevokeTherapeuticLinkRequest = requestObjectMapper.mapRevokeTherapeuticLinkRequest(RevokeTherapeuticLinkRequest(DateTime(), getNihii(therLink.hcParty)!!, makeAuthor(getNihii(therLink.hcParty), getSsin(therLink.hcParty), therLink.hcParty.firstName, therLink.hcParty.familyName), makeTherapeuticLink(therLink.type, therLink.hcParty, therLink.patient, therLink.startDate?.toDateTime(LocalTime.MIDNIGHT)
				?: DateTime(), therLink.endDate?.toDateTime(LocalTime.MIDNIGHT), therLink.comment), makeProof(sign
				?: false, therLink.patient, therLink.hcParty)))
	...
	private fun makeTherapeuticLink(therLinkType: String, hcParty: HcParty, patient: Patient, startDate: DateTime? = null, endDate: DateTime? = null, comment: String? = null): TherapeuticLink =
			TherapeuticLink(patient, hcParty, therLinkType).apply {
	...
	private fun makePatient(niss: String, eid: String?, isi: String?, firstName: String, lastName: String): Patient {
		return Patient.Builder().withEid(eid).withIsiPlus(isi).withFamilyName(lastName).withFirstName(firstName).withInss(niss).build()
	}
	...
	private fun makeProof(sign: Boolean, patient: Patient, hcParty: HcParty): Proof? {
		...
		return when {
			sign -> makeProofForEidSigning(patient, hcParty, BeIDCredential.getInstance("Therapeutic Link", "Signature"))
	...
	private fun makeHcParty(nihii: String?, inss: String?, firstname: String?, lastname: String?) =
			HcParty.Builder().withNihii(nihii).withInss(inss).withType("persphysician").withFirstName(firstname).withFamilyName(lastname).build()
	...
	internal fun getTherLinkPort(token: SAMLToken): GenericRequest =
			GenericRequest().setEndpoint(config.getProperty("endpoint.therlink"))
				.setCredential(token, TokenType.SAML)
				.addDefaulHandlerChain()
				.addHandlerChain(HandlerChainUtil.buildChainWithValidator("validation.incoming.therlink.message", "/ehealth-hubservices/XSD/hubservices_protocol-2_2.xsd"))
}