...
class DmgServiceImpl(private val stsService: STSService) : DmgService {
	...
	override fun registerDoctor(keystoreId: UUID, tokenId: UUID, passPhrase: String, hcpNihii: String, hcpSsin: String, hcpFirstName: String, hcpLastName: String, oa: String, bic: String, iban: String): DmgRegistration {
	...
	fun fillGmdRequest(gmdRequest : SendRequestType, isTest: Boolean, blob : Blob, credential: Credential, references: DMGReferences, hcpNihii: String, hcpSsin: String, hcpFirstName: String, hcpLastName: String, patientInfo: Patient, referenceDate: DateTime) {
			...
			commonInput = CommonInputType().apply {
				...
				this.origin = OriginType().apply {
					this.`package` = be.fgov.ehealth.globalmedicalfile.core.v1.PackageType().apply {
						this.name = be.fgov.ehealth.globalmedicalfile.core.v1.ValueRefString().apply { this.value = config.getProperty("genericasync.dmg.package.name") }
					...
					this.careProvider = be.fgov.ehealth.globalmedicalfile.core.v1.CareProviderType().apply {
						this.nihii = be.fgov.ehealth.globalmedicalfile.core.v1.NihiiType().apply {
							this.quality = "doctor"
							this.value = be.fgov.ehealth.globalmedicalfile.core.v1.ValueRefString().apply { this.value = hcpNihii }
						}
						this.physicalPerson = be.fgov.ehealth.globalmedicalfile.core.v1.IdType().apply {
							this.ssin = be.fgov.ehealth.globalmedicalfile.core.v1.ValueRefString().apply { this.value = hcpSsin }
						}
					}
				}
	...
	fun buildSendConsultRequest(isTest: Boolean, credential: Credential, references: DMGReferences, hcpNihii: String, hcpSsin: String, hcpFirstName: String, hcpLastName: String, patientInfo: Patient, referenceDate: DateTime, request: SelectRetrieveTransaction): ConsultGlobalMedicalFileRequest {
		val req = RetrieveTransactionRequest().apply {
			this.request = RequestType().apply {
				this.id = IDKMEHR().apply { this.s = IDKMEHRschemes.ID_KMEHR; this.sv = "1.0"; this.value = hcpNihii + "." + references.kmehrIdSuffix }
		...
		return ConsultGlobalMedicalFileRequest().apply {
			fillGmdRequest(this, isTest, blob, credential, references, hcpNihii, hcpSsin, hcpFirstName, hcpLastName, patientInfo, referenceDate)
		}
	}

	fun buildSendNotifyRequest(isTest: Boolean, credential: Credential, references: DMGReferences, hcpNihii: String, hcpSsin: String, hcpFirstName: String, hcpLastName: String, patientInfo: Patient, referenceDate: DateTime, kmehrMessage: Kmehrmessage): NotifyGlobalMedicalFileRequest {
	...
	override fun notifyDmg(keystoreId: UUID, tokenId: UUID, passPhrase: String, hcpNihii: String, hcpSsin: String, hcpFirstName: String, hcpLastName: String, patientSsin: String?, oa: String?, regNrWithMut: String?, patientFirstName: String, patientLastName: String, patientGender: String?, nomenclature: String, requestDate: Date): DmgNotification {
		...
		val kmehrmessage = Kmehrmessage().apply {
			...
			folders.add(FolderType().apply {
				...
				patient = PersonType().apply {
					...
					insurancymembership = regNrWithMut?.let { MemberinsuranceType().apply { membership = regNrWithMut; id = oa?.let { IDINSURANCE().apply { sv = "1.0"; value = it } } } }
					sex = patientGender?.let { SexType().apply { cd = CDSEX().apply { sv = "1.0"; value = CDSEXvalues.fromValue(it) } } }
				}
		...
		val response = ResponseObjectBuilderFactory.getResponseObjectBuilder().handleSendResponseType(org.taktik.connector.technical.ws.ServiceFactory.getGenericWsSender().send(GenericRequest().apply {
			setEndpoint(config.getProperty("endpoint.dmg.notification.v1", "\$uddi{uddi:ehealth-fgov-be:business:globalmedicalfilenotification:v1}"))
			setCredential(samlToken, TokenType.SAML)
			addDefaulHandlerChain()
			addHandlerChain(HandlerChainUtil.buildChainWithValidator("validation.incoming.message.dmg.notification.v1", "/ehealth-gmf/XSD/gmf_services_protocol-1_1.xsd"))
	...
	override fun consultDmg(keystoreId: UUID, tokenId: UUID, passPhrase: String, hcpNihii: String, hcpSsin: String, hcpFirstName: String, hcpLastName: String, patientSsin: String?, patientGender: String?, oa: String?, regNrWithMut: String?, requestDate: Date): DmgConsultation {
		var now = DateTime()
		assert(patientSsin != null || oa != null && regNrWithMut != null)
		val samlToken = stsService.getSAMLToken(tokenId, keystoreId, passPhrase)
				?: throw IllegalArgumentException("Cannot obtain token for Ehealth Box operations")
		...
		val response = ResponseObjectBuilderFactory.getResponseObjectBuilder().handleSendResponseType(org.taktik.connector.technical.ws.ServiceFactory.getGenericWsSender().send(GenericRequest().apply {
			setEndpoint(config.getProperty("endpoint.dmg.consultation.v1", "\$uddi{uddi:ehealth-fgov-be:business:globalmedicalfileconsultation:v1}"))
			setCredential(samlToken, TokenType.SAML)
			addDefaulHandlerChain()
			addHandlerChain(HandlerChainUtil.buildChainWithValidator("validation.incoming.message.dmg.consultation.v1", "/ehealth-gmf/XSD/gmf_services_protocol-1_1.xsd"))
	...
	override fun confirmDmgMessages(keystoreId: UUID, tokenId: UUID, passPhrase: String, hcpNihii: String, hcpSsin: String, hcpFirstName: String, hcpLastName: String, dmgMessages: List<DmgMessage>): Boolean {
		...
		val confirm = BuilderFactory.getRequestObjectBuilder("dmg").buildConfirmRequestWithHashes(
				buildOriginType(hcpNihii, hcpSsin, hcpFirstName, hcpLastName),
				dmgMessages.map { dmgMessage -> Base64.getDecoder().decode(dmgMessage.valueHash) }, listOf())
	...
	override fun confirmAcks(keystoreId: UUID, tokenId: UUID, passPhrase: String, hcpNihii: String, hcpSsin: String, hcpFirstName: String, hcpLastName: String, dmgTacks: List<DmgAcknowledge>): Boolean {
		if (dmgTacks.isEmpty()) {
			return true
		}
		val samlToken = stsService.getSAMLToken(tokenId, keystoreId, passPhrase)
		...
		val confirm = BuilderFactory.getRequestObjectBuilder("dmg").buildConfirmRequestWithHashes(
				buildOriginType(hcpNihii, hcpSsin, hcpFirstName, hcpLastName),
				listOf(), dmgTacks.map { ack -> Base64.getDecoder().decode(ack.valueHash) })
	...
	override fun getDmgMessages(keystoreId: UUID, tokenId: UUID, passPhrase: String, hcpNihii: String, hcpSsin: String, hcpFirstName: String, hcpLastName: String, oa: String, messageNames: List<String>?): List<DmgMessage> {
		val samlToken = stsService.getSAMLToken(tokenId, keystoreId, passPhrase)
				?: throw IllegalArgumentException("Cannot obtain token for Ehealth Box operations")
	...
	override fun postDmgsListRequest(keystoreId: UUID, tokenId: UUID, passPhrase: String, hcpNihii: String, hcpSsin: String, hcpFirstName: String, hcpLastName: String, oa: String?, requestDate: Date): Boolean {
		val samlToken = stsService.getSAMLToken(tokenId, keystoreId, passPhrase)
				?: throw IllegalArgumentException("Cannot obtain token for Ehealth Box operations")
