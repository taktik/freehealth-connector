...
data class Address(
    ...
    fun mergeFrom(other: Address) = Address(
        ...
        telecoms.addAll(this.telecoms.map { destTelecom ->
            other.telecoms.find { telecom -> telecom.telecomType === destTelecom.telecomType }?.let {
                destTelecom.mergeFrom(
                    it
                )
            } ?: destTelecom
        })
