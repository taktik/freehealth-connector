...
class ResponseObjectMapper {
    ...
    fun mapXMLToPutTherapeuticLinkResponse(xml: String) =
        this.generateJAXB(xml, be.fgov.ehealth.hubservices.core.v2.PutTherapeuticLinkResponse::class.java)?.let {
            PutTherapeuticLinkResponse(
                DateTime(
                    it.response.date.year,
                    it.response.date.monthOfYear,
                    it.response.date.dayOfMonth,
                    it.response.time.hourOfDay,
                    it.response.time.minuteOfHour,
                    it.response.time.secondOfMinute,
                    it.response.time.millisOfSecond,
                    DateTimeZone.getDefault()
                ),
                this.mapAuthor(it.response.author),
                it.response.id.value,
                this.mapOriginalPutTherapeuticLinkRequest(it.response.request),
                this.mapAcknowledge(it.acknowledge)
            )
        }
    ...
    fun mapXMLToRevokeTherapeuticLinkResponse(xml: String) =
        this.generateJAXB(xml, be.fgov.ehealth.hubservices.core.v2.RevokeTherapeuticLinkResponse::class.java)?.let {
            RevokeTherapeuticLinkResponse(
                ...
                this.mapAuthor(it.response.author),
                it.response.id.value,
                this.mapOriginalRevokeTherapeuticLinkRequest(it.response.request),
                this.mapAcknowledge(it.acknowledge)
            )
        }
    ...
    fun mapXMLToGetTherapeuticLinkResponse(xml: String) =
        this.generateJAXB(xml, be.fgov.ehealth.hubservices.core.v2.GetTherapeuticLinkResponse::class.java)?.let {
            GetTherapeuticLinkResponse(DateTime(
                it.response.date.year,
                it.response.date.monthOfYear,
                it.response.date.dayOfMonth,
                it.response.time.hourOfDay,
                ...
                DateTimeZone.getDefault()
                                       ...
                                       this.mapAuthor(it.response.author),
                                       it.response.id.value,
                                       this.mapOriginalGetTherapeuticLinkRequest(it.response.request),
                                       it.therapeuticlinklist?.therapeuticlinks?.map { mapTherapeuticLinkResponse(it) }
                                           ?: ArrayList(),
                                       this.mapAcknowledge(it.acknowledge))
        }
    ...
    fun mapJaxbToPutTherapeuticLinkResponse(jxbResp: be.fgov.ehealth.hubservices.core.v2.PutTherapeuticLinkResponse) =
        PutTherapeuticLinkResponse(
            mapDateTime(jxbResp.response.date, jxbResp.response.time),
            this.mapAuthor(jxbResp.response.author),
            jxbResp.response.id.value,
            this.mapOriginalPutTherapeuticLinkRequest(jxbResp.response.request),
            this.mapAcknowledge(jxbResp.acknowledge)
        )
    ...
    fun mapJaxbToGetTherapeuticLinkResponse(jxbResp: be.fgov.ehealth.hubservices.core.v2.GetTherapeuticLinkResponse) =
        GetTherapeuticLinkResponse(
            mapDateTime(jxbResp.response.date, jxbResp.response.time),
            mapAuthor(jxbResp.response.author),
            jxbResp.response.id.value,
            mapOriginalPutTherapeuticLinkRequest(jxbResp.response.request),
            jxbResp.therapeuticlinklist?.therapeuticlinks?.map { mapTherapeuticLinkResponse(it) } ?: ArrayList(),
            mapAcknowledge(jxbResp.acknowledge))
    ...
    private fun mapTherapeuticLinkResponse(therLink: TherapeuticLinkWithOperationContext) = TherapeuticLinkResponse(
        mapPatient(therLink.patient),
        therLink.hcparties.first().let { mapHcParty(it) },
        therLink.cd.value
    ).apply {
        ...
        operationContexts =
            ArrayList(therLink.operationcontexts.map {
                OperationContext(
                    it.recorddatetime,
                    it.operation.value(),
                    mapAuthor(it.author.author),
                    ArrayList(mapListOfProof(it.prooves))
                )
            })
    }

    private fun mapListOfProof(prooves: List<ProofType>) = prooves.map {
        Proof(it.cd.value).apply {
            binaryProof =
                if (it.binaryproof == null) null else BinaryProof(
                    it.binaryproof.cd.value.value(),
                    it.binaryproof.base64EncryptedValue.value
                )
        }
    }
    ...
    private fun mapHcParty(hcparty: HCPartyIdType) = HcParty().apply {
        ...
        ids.addAll(hcparty.ids.map {
            when (it.s) {
                ...
                IDHCPARTYschemes.ID_HCPARTY -> if ("application" == hcparty.cd.value) cbe = it.value else nihii =
                    it.value
    ...
    private fun mapHcParty(hcparty: HcpartyType) = HcParty().apply {
        ...
        hcparty.ids.forEach { id ->
            when {
                ...
                ("application_ID" == id.sl || "application id" == id.sl) && "application" == type -> applicationID =
                    id.value
    ...
    private fun mapOriginalPutTherapeuticLinkRequest(request: RequestType) = PutTherapeuticLinkRequest(
        DateTime(
            request.date.year,
            request.date.monthOfYear,
            request.date.dayOfMonth,
            request.time.hourOfDay,
            request.time.minuteOfHour,
            request.time.secondOfMinute,
            request.time.millisOfSecond
        ), request.id.value, this.mapAuthor(request.author)
    )

    private fun mapOriginalRevokeTherapeuticLinkRequest(request: RequestType) = RevokeTherapeuticLinkRequest(
        DateTime(
            request.date.year,
            request.date.monthOfYear,
            request.date.dayOfMonth,
    ...
    private fun mapOriginalGetTherapeuticLinkRequest(request: RequestType) = GetTherapeuticLinkRequest(
        DateTime(
            request.date.year,
            request.date.monthOfYear,
            request.date.dayOfMonth,
    ...
    private fun mapAuthor(authorType: AuthorType) =
        Author().apply { hcParties.addAll(authorType.hcparties.map { mapHcParty(it) }) }

    private fun mapDateTime(date: DateTime?, time: DateTime) = DateTime(
        date?.year ?: 0,
        date?.monthOfYear ?: 1,
        date?.dayOfMonth ?: 1,
        time.hourOfDay,
