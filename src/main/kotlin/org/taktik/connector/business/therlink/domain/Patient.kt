...
class Patient : org.taktik.connector.business.common.domain.Patient, Serializable {
	...
	class Builder {
		...
		fun build(): Patient {
			...
			val hasCompleteName = this.patient.firstName != null && this.patient.lastName != null && this.patient.name == null
			return if (!hasName && !hasCompleteName) {
				throw TherLinkBusinessConnectorException(TherLinkBusinessConnectorExceptionValues.VALIDATION_ERROR, *arrayOf<Any>("Patient should have a firstName and a FamilyName, (X)OR a name and nothing else"))
			} else {
				this.patient
			}
		}
