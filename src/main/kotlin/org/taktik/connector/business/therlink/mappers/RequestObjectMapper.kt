...
class RequestObjectMapper {
	...
	fun mapPutTherapeuticLinkToXML(request: PutTherapeuticLinkRequest) = this.generateXML(mapPutTherapeuticLinkRequest(request), be.fgov.ehealth.hubservices.core.v2.PutTherapeuticLinkRequest::class.java)
	fun mapRevokeTherapeuticLinkToXML(request: RevokeTherapeuticLinkRequest) = this.generateXML(this.mapRevokeTherapeuticLinkRequest(request), be.fgov.ehealth.hubservices.core.v2.RevokeTherapeuticLinkRequest::class.java)
	fun mapGetTherapeuticLinkToXml(request: GetTherapeuticLinkRequest) = this.generateXML(this.mapGetTherapeuticLinkRequest(request), be.fgov.ehealth.hubservices.core.v2.GetTherapeuticLinkRequest::class.java)

	fun mapPutTherapeuticLinkRequest(request: PutTherapeuticLinkRequest) = be.fgov.ehealth.hubservices.core.v2.PutTherapeuticLinkRequest().apply {
	...
	fun mapGetTherapeuticLinkRequest(request: GetTherapeuticLinkRequest) = be.fgov.ehealth.hubservices.core.v2.GetTherapeuticLinkRequest().apply {
		this.request = mapRequest(request.dateTime, request.id, request.author, if (request.maxRows != 0) request.maxRows else 999)
		select = mapGetTherapeuticLinkSelectType(request.link)
		prooves.addAll(mapProoves(request.proofs))
	}

	fun mapRevokeTherapeuticLinkRequest(request: TherapeuticLinkRequestType) = be.fgov.ehealth.hubservices.core.v2.RevokeTherapeuticLinkRequest().apply {
		this.request = mapRequest(request.dateTime, request.id, request.author, 0)
	...
	private fun mapIdsAndPropertiesToIdList(hcp: HcParty) = hcp.ids.plus(listOf(
			hcp.inss?.let { IDHCPARTY().apply { sv = "1.0"; s = IDHCPARTYschemes.INSS; value = it } },
			hcp.nihii?.let { IDHCPARTY().apply { sv = "1.0"; s = IDHCPARTYschemes.ID_HCPARTY; value = it } },
			hcp.cbe?.let { IDHCPARTY().apply { sv = "1.0"; s = IDHCPARTYschemes.ID_HCPARTY; value = it } },
			hcp.ehp?.let { IDHCPARTY().apply { sv = "1.0"; s = IDHCPARTYschemes.ID_HCPARTY; value = it } },
			hcp.applicationID?.let { IDHCPARTY().apply { sv = "1.0"; s = IDHCPARTYschemes.LOCAL; value = it; sl = "application_ID" } }
