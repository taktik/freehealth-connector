...
class HcParty : Serializable {
    ...
    override fun equals(obj: Any?): Boolean {
        if (this === obj) {
            ...
            return EqualsBuilder().append(this.cds, other!!.getCds()).append(this.ids, other.getIds())
                .append(this.familyName, other.familyName).append(this.firstName, other.firstName)
                .append(this.name, other.name).append(this.type, other.type).isEquals
        }
    }
    ...
    class Builder {
        ...
        private fun validateForApplicationRule() {
            if (this.hcp.applicationID != null && (!this.hcp.type!!.equals(
                    "application",
                    ignoreCase = true
                ) || this.hcp.nihii != null || this.hcp.inss != null)) {
                throw TherLinkBusinessConnectorException(
                    TherLinkBusinessConnectorExceptionValues.HCP_NOT_VALID,
                    "If HcParty has an applicationID, then type should be 'application' and niss neither nihii should be filled in"
                )
            }
        }
        ...
        private fun validateNameConstraint() {
            if (!this.hasName() && !this.hasCompleteName() && this.hasAtLeastOneName()) {
                throw TherLinkBusinessConnectorException(
                    ...
                    "Hcp should have a firstName and a FamilyName, (X)OR a name and nothing else"
                )
            }
        }
