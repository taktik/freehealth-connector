...
class RequestObjectBuilderImpl @Throws(WsConsentBusinessConnectorException::class, TechnicalConnectorException::class, InstantiationException::class)
constructor() : RequestObjectBuilder {
	private val log = LoggerFactory.getLogger(RequestObjectBuilderImpl::class.java)

	override fun createPutRequest(author: AuthorWithPatientAndPersonType?, consent: ConsentType?): PutPatientConsentRequest {
		if (author != null && consent != null) {
			...
			throw WsConsentBusinessConnectorException(WsConsentBusinessConnectorExceptionValues.REQUIRED_FIELD_NULL, "author and consent type  are required to create a PutPatientConsentRequest")
		}
	}

	override fun createGetRequest(author: AuthorWithPatientAndPersonType?, consent: SelectGetPatientConsentType?): GetPatientConsentRequest {
		if (author != null && consent != null) {
			...
			throw WsConsentBusinessConnectorException(WsConsentBusinessConnectorExceptionValues.REQUIRED_FIELD_NULL, "author and consent type are required to create a GetPatientConsentRequest")
		}
	}

	override fun createRevokeRequest(author: AuthorWithPatientAndPersonType?, consent: ConsentType?): RevokePatientConsentRequest {
		if (author != null && consent != null) {
			...
			throw WsConsentBusinessConnectorException(WsConsentBusinessConnectorExceptionValues.REQUIRED_FIELD_NULL, "author and consent type  are required to create a RevokePatientConsentRequest")
		}
	}
