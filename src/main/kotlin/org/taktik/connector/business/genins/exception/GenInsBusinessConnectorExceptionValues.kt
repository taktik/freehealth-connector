...
enum class GenInsBusinessConnectorExceptionValues(val errorCode: String, val message: String) {
    ...
    OBJECTBUILDER_INSTANCIATION_ERROR(
        "objectbuilder.instanciation.error",
    ...
    ERROR_XML_UNDEFINED_XSD_FOR_XML_CLASS_VALIDATOR(
        "error.xml.genins.undefined.class.validator",
        "no xsd file location is defined for class: {0}"
    ),
    ...
    TIMESTAMP_NOT_CORRECT(
        "error.xml.invalid.timestamp.in.response",
        "the timestamp in the response was not valid : {0}"
    )
}
