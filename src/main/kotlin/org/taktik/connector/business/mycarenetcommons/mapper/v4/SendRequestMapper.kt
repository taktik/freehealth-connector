...
class SendRequestMapper : ConfigurationModuleBootstrap.ModuleBootstrapHook {
    ...
    companion object {
        ...
        fun mapRouting(inRouting: Routing): RoutingType {
            ...
            calendar.time = (inRouting.referenceDate).toDate()
            val refDateTime = DatatypeFactory.newInstance().newXMLGregorianCalendar(calendar)
            routing.referenceDate = refDateTime
            return routing
        }
        ...
        private fun getPeriod(inPeriod: Period?): PeriodType? {
            ...
            if (inPeriod != null) {
                ...
                calendarBegin.time = (inPeriod.begin).toDate()
                val refDateBegin = DatatypeFactory.newInstance().newXMLGregorianCalendar(calendarBegin)
                period.start = refDateBegin

                val calendarEnd = GregorianCalendar()
                calendarEnd.time = (inPeriod.end).toDate()
                val refDateEnd = DatatypeFactory.newInstance().newXMLGregorianCalendar(calendarBegin)
                period.end = refDateEnd
            }
