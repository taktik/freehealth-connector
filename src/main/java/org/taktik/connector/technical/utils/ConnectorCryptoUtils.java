...
public final class ConnectorCryptoUtils {
   ...
   public static final String CONNECTORCRYPTO_ALGO_NAME_KEY = "org.taktik.connector.technical.utils.connectorcryptoutils.default_algo_name";
   public static final String CONNECTORCRYPTO_KEYSIZE_KEY = "org.taktik.connector.technical.utils.connectorcryptoutils.default_keysize";
   public static final String CONNECTORCRYPTO_MOCK_KEY = "org.taktik.connector.technical.utils.connectorcryptoutils.mock.desede";
   ...
   public static SecretKey generateKey(int keySize) throws TechnicalConnectorException {
      String algo = conf.getProperty("org.taktik.connector.technical.utils.connectorcryptoutils.default_algo_name", "AES");
   ...
   public static byte[] decrypt(Key key, String algo, byte[] encryptedBytes) throws TechnicalConnectorException {
      ...
      try {
         ...
         try {
            decrypt(encryptedBytes, baos, index, blockSize, new ConnectorCryptoUtils.SinglePartOperation(cipher));
         } catch (Exception var12) {
            LOG.debug("Not a SinglePart operation cipher. Trying MultiPartOperation. Reason [" + ExceptionUtils.getRootCauseMessage(var12) + "]", var12);
            baos.reset();
            decrypt(encryptedBytes, baos, index, blockSize, new ConnectorCryptoUtils.MultiPartOperationDecryptor(cipher));
         }
         ...
         throw new TechnicalConnectorException(errorValue, var13, new Object[]{param});
      } finally {
   ...
   private static class MultiPartOperationDecryptor implements ConnectorCryptoUtils.Decryptor {
   ...
   private static class SinglePartOperation implements ConnectorCryptoUtils.Decryptor {
      private Cipher cipher;
