...
class TherLinkControllerTest : EhealthTest() {
    ...
    fun getTherLink() {
        ...
        val therLink = this.restTemplate.getForObject("http://localhost:$port/therlink/${"74010414733"}/${"11478761004"}?keystoreId=$keystoreId&tokenId=$tokenId&hcpSsin=${ssin1}&hcpFirstName=${"Antoine"}&hcpLastName=${"Baudoux"}&patientSsin=${"74010414733"}&patientFirstName=${"Antoine"}&patientLastName=${"Duchâteau"}&passPhrase=$passPhrase", String::class.java)
    ...
    fun doesTherLinkExist() {
        ...
        assertThat(tlmsgs.size).isGreaterThan(0)
        val tlmsg = tlmsgs.first()
        val exists = this.restTemplate.postForObject("http://localhost:$port/therlink/check?keystoreId=$keystoreId&tokenId=$tokenId&&passPhrase=$passPhrase", tlmsg.therapeuticLink!!, String::class.java)
    ...
    fun revokeTherLink() {
        ...
        val tlmsg = tlmsgs.first()
        assertThat(tlmsg.therapeuticLink?.patient?.inss).isEqualToIgnoringCase("74010414733")
        val revoke = this.restTemplate.postForObject("http://localhost:$port/therlink/revoke?keystoreId=$keystoreId&tokenId=$tokenId&&passPhrase=$passPhrase", tlmsg.therapeuticLink!!, String::class.java)
        assertThat(revoke.length).isGreaterThan(0)
        //Recreate link
         this.restTemplate.postForObject("http://localhost:$port/therlink/register?keystoreId=$keystoreId&tokenId=$tokenId&hcpNihii=${"11478761004"}&hcpSsin=${ssin1}&hcpFirstName=${"Antoine"}&hcpLastName=${"Baudoux"}&patientSsin=${"74010414733"}&patientFirstName=${"Antoine"}&patientLastName=${"Duchâteau"}&eidCardNumber=${"592363302467"}&passPhrase=$passPhrase", null, String::class.java)
    }

    @Test
    fun registerTherLink() {
        val (keystoreId, tokenId, passPhrase) = register(restTemplate!!, port, ssin1!!, password1!!)
        val therLink = this.restTemplate.postForObject("http://localhost:$port/therlink/register?keystoreId=$keystoreId&tokenId=$tokenId&hcpNihii=${"11478761004"}&hcpSsin=${ssin1}&hcpFirstName=${"Antoine"}&hcpLastName=${"Baudoux"}&patientSsin=${"74010414733"}&patientFirstName=${"Antoine"}&patientLastName=${"Duchâteau"}&eidCardNumber=${"592363302467"}&passPhrase=$passPhrase", null, String::class.java)
        assertThat(therLink != null)
