...
public class HospitalExecutorIntegrationModuleV4Impl extends AbstractExecutorIntegrationModuleV4 {
	 ...
	 */
	public HospitalExecutorIntegrationModuleV4Impl() {
		super();
	}
	 ...
	 */
	private SecuredContentType createSecuredContentType(final byte[] content) {
	...
	@Override
	public GetPrescriptionStatusResult getData(final GetPrescriptionStatusParam param) {// 222
	...
	public void createFeedback(final String prescriberId, final String rid, final byte[] feedbackText) {
		...
		try {
			...
			final List<EncryptionToken> etkRecipients = getEtkHelper().getEtks(KgssIdentifierType.NIHII_HOSPITAL, prescriberId);
	...
	public ListReservationsResult getData(final ListReservationsParam param) {
		...
		try {
			try {
				final ListReservationsResult completeResult = new ListReservationsResult();
				...
				do {
					...
					if (param.getPage() != null && param.getPage().getPageNumber() == null) {
						currentPage = BigInteger.valueOf(0);
					} else {
					...
					result = unsealListReservationsResponse(response);
					checkStatus(result);
	 ...
	 */
	protected PutRidsInProcessRequest getPutAllRidInProcess(final PutRidsInProcessParam putRidsInProcessParam) {
