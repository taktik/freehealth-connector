...
/**
 * The Class HospitalPrescriberIntegrationModuleDevV4Impl.
 */
public class HospitalPrescriberIntegrationModuleDevV4Impl extends AbstractPrescriberIntegrationModule implements PrescriberIntegrationModuleDevV4 {
	...
	public void sendNotification(final byte[] notificationText, final String patientId, final String executorId) {
		...
		try {
			...
			try {
				etkRecipients.addAll(getEtkHelper().getEtks(KgssIdentifierType.NIHII, executorId));
			} catch (Exception e) {
				try {
					etkRecipients.addAll(getEtkHelper().getEtks(KgssIdentifierType.NIHII_PHARMACY, executorId));
				} catch (Exception e1) {
					try {
						etkRecipients.addAll(getEtkHelper().getEtks(KgssIdentifierType.NIHII_HOSPITAL, executorId));
					} catch (Exception e2) {
