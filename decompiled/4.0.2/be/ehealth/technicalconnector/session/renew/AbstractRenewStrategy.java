...
public abstract class AbstractRenewStrategy implements RenewStrategy {
      ...
      try {
         if (session.getHeaderCredential() != null && config.getBooleanProperty("sessionmanager.activate.autorenew", false)) {
            synchronized(mutex) {
               ...
               Iterator var8 = cacheServices.iterator();

               while(var8.hasNext()) {
                  SessionServiceWithCache serviceWithCache = (SessionServiceWithCache)var8.next();
   ...
   public void flushCache() {
      Iterator var1 = this.cacheServices.iterator();

      while(var1.hasNext()) {
         SessionServiceWithCache service = (SessionServiceWithCache)var1.next();
         service.flushCache();
      }
