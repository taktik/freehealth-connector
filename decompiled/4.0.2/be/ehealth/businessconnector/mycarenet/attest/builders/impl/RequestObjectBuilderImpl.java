...
public class RequestObjectBuilderImpl implements RequestObjectBuilder, ConfigurationModuleBootstrap.ModuleBootstrapHook {
   ...
   private void buildAttestationRequest(boolean isTest, InputReference references, Ssin patientSsin, DateTime referenceDate, SendAttestationRequest sendAttestationRequest, Blob blob) throws TechnicalConnectorException {
      ...
      sendAttestationRequest.setCommonInput(((CommonInputMapper)Mappers.getMapper(CommonInputMapper.class)).map(commonBuilder.createCommonInput(McnConfigUtil.retrievePackageInfo("attest"), isTest, references.getInputReference())));
      ...
      sendAttestationRequest.setRouting(((RoutingMapper)Mappers.getMapper(RoutingMapper.class)).map(commonBuilder.createRouting(patient, referenceDate)));
