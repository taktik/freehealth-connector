...
public class ResponseObjectMapper {
   ...
   protected ResponseObjectMapper() {
   }
   ...
   private List<TherapeuticLinkResponse> mapListOfTherapeuticLinks(TherapeuticLinkListType therapeuticlinklist) {
      if (therapeuticlinklist == null) {
         ...
         Iterator var3 = therapeuticlinklist.getTherapeuticlinks().iterator();

         while(var3.hasNext()) {
            TherapeuticLinkWithOperationContext therLink = (TherapeuticLinkWithOperationContext)var3.next();
   ...
   private List<OperationContext> mapListOfOperationContext(List<OperationContextType> operationContexts) {
      ...
      Iterator var3 = operationContexts.iterator();

      while(var3.hasNext()) {
         OperationContextType opContextType = (OperationContextType)var3.next();
   ...
   private List<Proof> mapListOfProof(List<ProofType> prooves) {
      ...
      Iterator var3 = prooves.iterator();

      while(var3.hasNext()) {
         ProofType proofType = (ProofType)var3.next();
   ...
   private HcParty mapHcParty(HCPartyIdType hcparty) {
      ...
      Iterator var3 = hcparty.getIds().iterator();

      while(var3.hasNext()) {
         IDHCPARTY id = (IDHCPARTY)var3.next();
   ...
   private Patient mapPatient(PatientIdType patientIdType) {
      ...
      Iterator var3 = patientIdType.getIds().iterator();

      while(var3.hasNext()) {
         IDPATIENT id = (IDPATIENT)var3.next();
   ...
   private void mapAuthor(AuthorType authorType, Author author) {
      Iterator var3 = authorType.getHcparties().iterator();

      while(var3.hasNext()) {
         HcpartyType hcpType = (HcpartyType)var3.next();
   ...
   private HcParty mapHcParty(HcpartyType hcpType) {
      ...
      Iterator var4 = hcpType.getIds().iterator();

      while(true) {
         while(true) {
            while(var4.hasNext()) {
               IDHCPARTY id = (IDHCPARTY)var4.next();
