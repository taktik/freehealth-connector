...
public final class ResponseBuilderImpl implements ResponseBuilder, ConfigurableImplementation, ConfigurationModuleBootstrap.ModuleBootstrapHook {
   ...
   private void commonValidateMethod(SendResponseType responseType, Blob blob) throws InvalidBlobContentConnectorException, TechnicalConnectorException {
      ...
      this.validate(responseType, blob, xades);
   }

   private void commonValidateMethod(be.fgov.ehealth.mycarenet.commons.protocol.v2.SendResponseType responseType, Blob blob) throws InvalidBlobContentConnectorException, TechnicalConnectorException {
      Base64Binary xades = null;
      if (responseType != null && responseType.getReturn() != null) {
         xades = responseType.getReturn().getXadesT();
      }
   ...
   private void validate(Object responseType, Blob blob, Base64Binary xades) throws TechnicalConnectorException {
      this.checkIfInitialized();
      ...
      if (xades != null && xades.getValue() != null && !ArrayUtils.isEmpty(xades.getValue())) {
         ...
         SignatureVerificationResult result = builder.verify((byte[])this.reassemblyMessage(responseType, sigElement), (byte[])xadesByteArray, optionMap);
   ...
   public void createInvalidBlobContentConnectorException(Blob blob, SignatureVerificationResult result, Base64Binary xades) throws InvalidBlobContentConnectorException {
      ...
      Iterator var5 = result.getErrors().iterator();

      while(var5.hasNext()) {
         SignatureVerificationError verificationError = (SignatureVerificationError)var5.next();
   ...
   public String getResponse(be.fgov.ehealth.mycarenet.commons.protocol.v2.SendResponseType responseType) throws TechnicalConnectorException {
      return this.getResponse(responseType, true);
   }
   ...
   public String getResponse(be.fgov.ehealth.mycarenet.commons.protocol.v2.SendResponseType responseType, boolean validate) throws TechnicalConnectorException {
      Blob blob = this.mapBlobTypeToBlob(responseType.getReturn().getDetail());
      if (validate) {
         CommonOutputValidator.validate(responseType.getReturn().getCommonOutput());
         this.validateHash(blob);
   ...
   public Blob mapBlobTypeToBlob(be.fgov.ehealth.mycarenet.commons.core.v2.BlobType inBlob) {
      Blob blob = new Blob();
      blob.setId(inBlob.getId());
      blob.setContent(inBlob.getValue());
      blob.setHashValue(inBlob.getHashValue());
