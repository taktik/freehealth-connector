...
public final class HcPartyUtil {
   ...
   public static void addSecurityTags(AuthorType author, String projectName) throws TechnicalConnectorException {
      Iterator var2 = author.getHcparties().iterator();

      while(var2.hasNext()) {
         HcpartyType party = (HcpartyType)var2.next();
         boolean found = false;
         Iterator var5 = party.getCds().iterator();

         while(var5.hasNext()) {
            CDHCPARTY partyType = (CDHCPARTY)var5.next();
         ...
         if (found) {
            addEncryptionActor(party);
            addEncryptionApplication(party);
            addEncryptionReplyToEtkBase64(party, projectName);
         }
      }
   ...
   public static void addSecurityTags(AuthorType author) throws TechnicalConnectorException {
      addSecurityTags(author, (String)null);
   }

   private static void addEncryptionActor(HcpartyType hcParty) throws TechnicalConnectorException {
      ...
      String nonDefaultCdEncryptionActor = ConfigFactory.getConfigValidator().getProperty("encryption.actor.cd");
      if (StringUtils.isNotBlank(nonDefaultCdEncryptionActor)) {
         IdentifierType identifierType = IdentifierType.valueOf(nonDefaultCdEncryptionActor);
         if (identifierType == null) {
            throw new TechnicalConnectorException(TechnicalConnectorExceptionValues.INVALID_PROPERTY_VALUE, new Object[]{nonDefaultCdEncryptionActor, " not a valid encryption.actor.cd property"});
         }

         cdBuilder.value(identifierType.getType(48));
         String idEncryptionActor = ConfigFactory.getConfigValidator().getProperty("encryption.actor.id");
         if (StringUtils.isBlank(idEncryptionActor)) {
            throw new TechnicalConnectorException(TechnicalConnectorExceptionValues.INVALID_PROPERTY_VALUE, new Object[]{idEncryptionActor, " it should not be null or empty"});
         }

         idBuilder.value(ConfigFactory.getConfigValidator().getProperty("encryption.actor.id"));
   ...
   private static void addEncryptionApplication(HcpartyType hcParty) throws TechnicalConnectorException {
      String idEncryptionApplication = ConfigFactory.getConfigValidator().getProperty("encryption.application.id");
      if (idEncryptionApplication != null) {
         Id idApplicationBuilder = (new Id()).s(IDHCPARTYschemes.ID_ENCRYPTION_APPLICATION).sv("1.0");
         idApplicationBuilder.value(idEncryptionApplication);
