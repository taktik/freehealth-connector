...
public final class TrustServiceStatusListSignatureVerifier {
   ...
   private static void dumpTsloStore() {
      if (LOG.isDebugEnabled()) {
         try {
            ...
            Iterator var1 = tsloCerts.iterator();

            while(var1.hasNext()) {
               Certificate tsloCert = (Certificate)var1.next();
   ...
   private static CertStore getCertStore() throws Exception {
      ...
      try {
         KeyStore tslStore = KeyStore.getInstance(ConfigFactory.getConfigValidator().getProperty("be.fgov.ehealth.technicalconnector.bootstrap.tsl.keystore.type", "JKS"));
         tslStore.load(ConnectorIOUtils.getResourceAsStream(ConfigFactory.getConfigValidator().getProperty("be.fgov.ehealth.technicalconnector.bootstrap.tsl.keystore.location")), ConfigFactory.getConfigValidator().getProperty("be.fgov.ehealth.technicalconnector.bootstrap.tsl.keystore.pwd", "").toCharArray());
         ...
         LOG.error("Error while loading keystore", var5);
      }
