...
public class ApplicationConfig {
   ...
   public void initialize(String propertyfile, String validationFile) throws IntegrationModuleException {
      this.initialize(propertyfile, validationFile, (String)null);
   }
   ...
   private void getLatestTIPConfiguration() throws IntegrationModuleException {
      if (SessionValidator.isValidSession(Session.getInstance().getSession())) {
         TipConfigModuleImpl tipConfigModule = new TipConfigModuleImpl();

         try {
            tipConfigModule.getLatestTIPConfig();
         } catch (Exception var3) {
            LOG.warn("Failed to retrieve latest tip configuration due to exception: ", var3);
         }
      } else {
   ...
   public void checkLatestTIPConfiguration() throws IntegrationModuleException {
      if (PropertyHandler.getInstance().hasProperty("care.provider.type") && PropertyHandler.getInstance().getProperty("care.provider.type").toUpperCase().equals(CareProviderType.PHARMACIST.toString())) {
         this.initProductFilterSingleton();
         this.getLatestTIPConfiguration();
      }
