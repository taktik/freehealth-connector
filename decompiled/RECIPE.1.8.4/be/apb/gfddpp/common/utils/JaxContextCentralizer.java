...
public class JaxContextCentralizer {
   ...
   public final JAXBContext getContext(Class<?> clazz) throws GFDDPPException {
      if (!this.contextStore.containsKey(clazz)) {
         try {
            this.contextStore.put(clazz, JAXBContext.newInstance(new Class[]{clazz}));
   ...
   public <X> X toObject(Class<X> clazz, byte[] data) throws GFDDPPException {
      try {
         ...
         if (clazz.getAnnotation(XmlRootElement.class) != null) {
            result = this.getUnmarshaller(clazz).unmarshal(bis);
         } else {
            try {
               JAXBElement<X> jax = this.getUnmarshaller(clazz).unmarshal(this.xmlInputFactory.createXMLStreamReader(bis, "UTF-8"), clazz);
               result = jax.getValue();
