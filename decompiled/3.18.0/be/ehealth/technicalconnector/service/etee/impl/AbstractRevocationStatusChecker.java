...
public abstract class AbstractRevocationStatusChecker implements RevocationStatusChecker, SessionServiceWithCache {
   ...
   public AbstractRevocationStatusChecker() {
      this.cache = CacheFactory.newInstance(CacheFactory.CacheType.MEMORY, "revocation-cache", CacheInformation.ExpiryType.NONE, (Duration)null);
      ...
      if (cert == null) {
         ...
         if (!this.cache.containsKey(cert)) {
            if (LOG.isInfoEnabled()) {
               LOG.info("Checking revocation status for cert from subject : {}", cert.getSubjectX500Principal());
            }
