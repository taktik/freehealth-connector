...
public class ErrorCollectorHandler implements ErrorHandler {
   ...
   public final boolean hasExceptions(String... errorType) {
      if (ArrayUtils.contains(errorType, "WARN") && !this.exceptionWarningList.isEmpty()) {
         return true;
      } else if (ArrayUtils.contains(errorType, "ERROR") && !this.exceptionErrorList.isEmpty()) {
         return true;
      } else {
         return ArrayUtils.contains(errorType, "FATAL") && !this.exceptionFatalList.isEmpty();
      }
   }
