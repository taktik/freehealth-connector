...
class ServerImpl implements TimeSource {
   ...
   public void start() {
      if (this.bound && !this.started && !this.finished) {
         if (this.executor == null) {
            this.executor = new ServerImpl.DefaultExecutor((ServerImpl.SyntheticClass_1)null);
         }
   ...
   static class SyntheticClass_1 {
      // $FF: synthetic field
      static final int[] $SwitchMap$sun$net$httpserver$HttpConnection$State = new int[HttpConnection.State.values().length];

      static {
         try {
            $SwitchMap$sun$net$httpserver$HttpConnection$State[HttpConnection.State.REQUEST.ordinal()] = 1;
         } catch (NoSuchFieldError var3) {
            ;
         }

         try {
            $SwitchMap$sun$net$httpserver$HttpConnection$State[HttpConnection.State.RESPONSE.ordinal()] = 2;
         } catch (NoSuchFieldError var2) {
            ;
         }

         try {
            $SwitchMap$sun$net$httpserver$HttpConnection$State[HttpConnection.State.IDLE.ordinal()] = 3;
   ...
   private static class DefaultExecutor implements Executor {
      ...
      // $FF: synthetic method
      DefaultExecutor(ServerImpl.SyntheticClass_1 x0) {
         this();
      }
   }
}
