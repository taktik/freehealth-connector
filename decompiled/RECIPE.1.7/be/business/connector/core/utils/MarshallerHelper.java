...
public class MarshallerHelper<X, Y> {
   ...
   public X toObject(byte[] data) {
      try {
         ...
         X result = this.unmarshaller.unmarshal((InputStream)bis);
         bis.close();
   ...
   public X unmarsh(String data) throws JAXBException {
      return this.unmarshaller.unmarshal((Reader)(new StringReader(data)));
   }

   public X unmarsh(byte[] data) throws IntegrationModuleException {
      try {
         return this.unmarshaller.unmarshal((InputStream)(new ByteArrayInputStream(data)));
      } catch (JAXBException var3) {
