...
public class CertificateCallback extends AbstractWsSecurityHandler {
   ...
   protected void addWSSecurity(SOAPMessageContext context) throws TechnicalConnectorException {
      ...
      this.buildSignature().on(context.getMessage()).withTimeStamp(this.getTimeStampTTL(), TimeUnit.SECONDS).withBinarySecurityToken(lazyCred).sign(new AbstractWsSecurityHandler.SignedParts[]{AbstractWsSecurityHandler.SignedParts.BODY, AbstractWsSecurityHandler.SignedParts.TIMESTAMP, AbstractWsSecurityHandler.SignedParts.BST});
   }
